...
d.append(draw.Rectangle(r_x, r_y, r_w, r_h, rx=18, ry=18, 
                       fill='#EEF4FF', stroke='#3E63DD', stroke_width=3))

# Generator box - secondary accent color  
d.append(draw.Rectangle(g_x, g_y, g_w, g_h, rx=18, ry=18,
                       fill='#FFF6EE', stroke='#DDB88E', stroke_width=3))

# Document Store box - neutral with subtle tint
d.append(draw.Rectangle(d_x, d_y, d_w, d_h, rx=18, ry=18,
                       fill='#F8F9FA', stroke='#868E96', stroke_width=3))

# Labels with better typography and colors
d.append(draw.Text('Retriever', 32, r_cx, r_cy + 10, center=True, 
                  fill='#3E63DD', font_weight='600'))
d.append(draw.Text('Generator', 32, g_cx, g_cy + 10, center=True, 
                  fill='#DDB88E', font_weight='600'))
d.append(draw.Text('Document', 28, d_x + d_w/2, d_y + 40, center=True, 
                  fill='#495057', font_weight='500'))
d.append(draw.Text('Store', 28, d_x + d_w/2, d_y + 78, center=True, 
                  fill='#495057', font_weight='500'))
...
def line_arrow(p1, p2, color='#3E63DD', width=4):
    x1, y1 = p1; x2, y2 = p2
    d.append(draw.Line(x1, y1, x2, y2, stroke=color, stroke_width=width, 
                      marker_end=arrow, stroke_linecap='round'))
...
curve = draw.Path(stroke='#2C4DBA', fill='none', stroke_width=4, 
                 marker_end=arrow, stroke_linecap='round')
